...
"""
Calculate Task Balanced Multimodal Feature Selection objective loss
"""
function objloss!(obj𝓛s, i, Xₘ, Y_hot, W, b, Z, Bₘ, αₘ, γₘ, C)
    ...
    obj𝓛s[i] += C * norm(W)^2
    obj𝓛s[i] += sum(max.(1 .- (W' * Z .+ b) .* Y_hot, 0))
end

"""
Calculates the Lagrangian loss
"""
...
function init_var(model::TBMFSClassifier, X, Y)
    ...
    Xₘ = [vcat(xₘ, ones(size(xₘ)[2])') for xₘ in Xₘ] # Add intercept to each modality. Perhaps Z instead?
    ...
    Bₘ = [randn(model.r, dₘ[i]) for i in 1:M]
    B̂ₘ = [randn(model.r, dₘ[i]) for i in 1:M]

    Θₘ = [zeros(dₘ[i], N) for i in 1:M]
    Λₘ = [zeros(model.r, dₘ[i]) for i in 1:M]
    Ω = zeros(model.r, N)
    η = zeros(K, N)
...
function update_E!(E, Y_hot, W, b, Z, η, μ)
    S = Y_hot .- (W'*Z  .+ b) - η/μ
...
function update_W!(W, Y_hot, E, b, Z, η, C, μ)
    _, K = size(W)
    r, N = size(Z)
    ∑zᵢzᵢᵀ = zeros(r, r)
    ...
    W[:] = (((Y_hot .- E .- b .- η/μ) * Z') * inv(∑zᵢzᵢᵀ + C*I))'
end
...
function update_Z!(Z, Xₘ, Y_hot, E, W, b, Ẑ, Fₘ, Bₘ, Θₘ, Ω, η, μ)
    ...
    Z[:] = inv(∑BₘBₘᵀ + ∑wₖwₖᵀ + I) * (∑BₘT + ∑wₖU + Ẑ - Ω/μ)
end
...
function update_Bₘ!(Bₘ, Xₘ, Z, Fₘ, B̂ₘ, Θₘ, Λₘ, μ)
    Bₘ[:] = (Z*Z' + I) \ (-Z*(Fₘ'-Xₘ'+Θₘ'/μ) + (B̂ₘ - Λₘ/μ))
end
...
function MLJBase.predict(model::TBMFSClassifier, fitresult, Xnew)
    ...
    for m in 1:length(Xnewₘ)
        BT = Matrix(Bₘ[m]')
        Zpartial = model.αₘ[m] * pinv(BT) * Xnewₘ[m]
        if DEBUG
            display(Zpartial)
        end
        Znew += Zpartial
